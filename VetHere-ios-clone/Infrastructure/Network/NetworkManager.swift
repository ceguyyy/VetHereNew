...
public class NetworkManager {
  ...
  func makeRequest<T: Decodable>(_ request: APIService, output: T.Type) async -> Result<
    BaseResponse<T>, NetworkError
  > {
        ...
        case .multiformdata:
          ...
          for (key, value) in parameters {
            if let valueString = value as? String {
              appendFormField(name: key, value: valueString, to: &body, boundary: boundary)
            }
          }
    ...
    do {
      ...
      if httpResponse.statusCode == 401 && !request.path.contains("refresh")
        && !request.path.contains("login") && !request.path.contains("register")
      {
  ...
  private func appendFileField(
    name: String, data: Data, to body: inout Data, boundary: String, mimeType: String,
    filename: String
  ) {
